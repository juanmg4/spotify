...
export class SearchComponent implements OnInit {
  ...
  constructor( private _spotify: SpotifyService, public _favoritos: FavoritosService) { } //si el servicio es publico, puedo usar sus metodos y atributos directamente en el html
  ...
  buscar(contenidoBusqueda: string){
    ...
    this._spotify.getArtistas( contenidoBusqueda.substring(1) ).subscribe( (data:any) => {
    ...
    /* si la primera letra del formulario no es un arroba busca canciones*/
      ...
      let cargarCancionesBuscadas = (callback)=>{
      this.buscarArtista=false;
      this._spotify.getTracks( contenidoBusqueda ).subscribe( (data:any) => {
    ...
    /* defino lo que hace el callback */
    let callback = () => {
...
eliminarArtistaFavorito(posicionArtistaFavorito:number){
  this._favoritos.obtenerArtistasFavoritos().splice(posicionArtistaFavorito,1);
}
eliminarCancionFavorita(posicionCancionFavorita:number){
  this._favoritos.obtenerCancionesFavoritas().splice(posicionCancionFavorita,1);
}
eliminarNuevoLanzamientoFavorito(posicionNuevoLanzamientoFavorito:number){
...
ModificarFecha(){
 ...
 /* solo de las canciones mas nuevas */
